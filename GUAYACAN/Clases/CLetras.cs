...
namespace GUAYACAN
{
	...
	public class CantidadLetra
	{
		...
		public string ConvertirCadena (string sNumero) {
            ...
            {
                //sResultado += ObtenerDecimales(sAux);
                string[] Cantidad = sAux.Split('.');
                int auxDecimal = Convert.ToInt32(Cantidad[1]);
                if (auxDecimal < 10)
                {
                    sResultado += "PESOS " + auxDecimal.ToString() + "0/100 M.N.";
                }
                else
                {
                    sResultado += "PESOS " + auxDecimal.ToString() + "/100 M.N.";
                }
...
//				sNumPuntos[1] = sNumPuntos[1].Remove (3, sNumPuntos[1].Length -3);
			...
			sTexto = "PESOS" + Numeros(dNumero,1) + "/100";
